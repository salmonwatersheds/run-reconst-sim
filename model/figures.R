...
# quartz(width = 5, height= 4.5, pointsize = 10)
# # layout(matrix(c(1,2,3,3), nrow=2, byrow=2))
# par(mfrow=c(1,2), mar=c(4,4,2,1))
# 
# # Plot status outcomes for base case of a = 1.4
# i <- 2
# 
# plotStatusDiff(out_base[[i]]$SR[, 3])
# # mtext(side=3, "Stock-recruitment", line=1)
# mtext(side=3, adj=0, line=0.5, "a) Stock-recruitment")
# 
# plotStatusDiff(out_base[[i]]$HS[,3])
# # mtext(side=3, "Historic spawners", line=1)
# mtext(side=3, adj=0, line=0.5, "b) Historical spawners")
# 
#------------------------------------------------------------------------------
i <- 2
...
# #------------------------------------------------------------------------------
# # Why misclassifications?
PE <- cbind(avgS = out_base[[i]]$avgS, Sgen1 = out_base[[i]]$SR[, 1], Smsy = out_base[[i]]$SR[, 2],  S25 = out_base[[i]]$HS[, 1], S50 = out_base[[i]]$HS[, 2])
apply(PE, 2, quantile, c(0.25, 0.5, 0.75), na.rm=TRUE) 

par(mfrow=c(1,2), mar=c(4,4,2,1))
boxplot(PE[,c(2,1,3)], names=c(expression(italic(S[GEN1])), expression(italic(S[AVG])), expression(paste("80%", italic(S[MSY])))), outline=FALSE, col=c(grey(0.5), "white", grey(0.5)), las=1, ylab="", yaxt="n", ylim=c(-1, 3))
abline(h=0)
A <- axis(side=2, labels = FALSE)
axis(side=2, at = A, labels = paste(formatC(round(A*100, 1), 0, format="f"), "%", sep=""), las=1)
mtext(side=2, line=4, "Percentage relative bias")
# mtext(side=3, adj=0, line=0.5, "c)")

boxplot(PE[,c(4,1,5)], names=c(expression(italic(S[25])), expression(italic(S[AVG])), expression(italic(S[50]))), outline=FALSE, col=c(grey(0.5), "white", grey(0.5)), las=1, ylab="", yaxt="n", ylim=c(-1, 6))
abline(h=0)
A <- axis(side=2, labels = FALSE)
axis(side=2, at = A, labels = paste(formatC(round(A*100, 1), 0, format="f"), "%", sep=""), las=1)
mtext(side=2, line=4, "Percentage relative bias")
...
plotCI(x - jit, delistedOut$RB$Sgen1[, 'median'], "n", li = delistedOut$RB$Sgen1[, '25th'], ui= delistedOut$RB$Sgen1[, '75th'], yaxt="n", col=NA, ylab="", xlab="", xaxt="n", xlim=c(0.5, 4.5), ylim=c(-0.2, 1.2))#range(yAll))
...
plotCI(x - jit, delistedOut$RB$Sgen1[, 'median'], "n", li = delistedOut$RB$Sgen1[, '25th'], ui= delistedOut$RB$Sgen1[, '75th'], gap=0, sfrac = 0, col=statusCols['r'], cex=0.8, pch=25, pt.bg="white", add=TRUE)

plotCI(x + jit, delistedOut$RB$Smsy[, 'median'], li = delistedOut$RB$Smsy[, '25th'], ui=delistedOut$RB$Smsy[, '75th'], gap=0, sfrac = 0, add=TRUE, col=statusCols['g'], cex=0.8, pch=24, pt.bg="white")

plotCI(x, delistedOut$RB$avgS[, 'median'], li = delistedOut$RB$avgS[, '25th'], ui= delistedOut$RB$avgS[, '75th'], gap=0, sfrac = 0, add=TRUE, cex=0.8, pch=21, pt.bg="white")

A <- axis(side=2, labels = FALSE)
axis(side=2, at = A, labels = paste(formatC(round(A*100, 1), 0, format="f"), "%", sep=""), las=1)
mtext(side=2, line=4, "Relative bias")

legend(3, u[4] + 0.22*(u[4]-u[3]), pch = c(25, 21, 24), col=c(statusCols['r'], 1, statusCols['g']), c(expression(italic(S[GEN1])), expression(italic(S[AVG])), expression(paste("80%", italic(S[MSY])))), pt.cex = 0.8, bg="white", xpd=NA)
...
plotCI(x - jit, delistedOut$RB$S25[, 'median'], "n", li = delistedOut$RB$S25[, '25th'], ui= delistedOut$RB$S25[, '75th'], yaxt="n", col=NA, ylab="", xlab="", xaxt="n", ylim=c(-0.2, 3.5), xlim=c(0.5, 4.5))
...
plotCI(x - jit, delistedOut$RB$S25[, 'median'], "n", li = delistedOut$RB$S25[, '25th'], ui= delistedOut$RB$S25[, '75th'], gap=0, sfrac = 0, col=statusCols['r'], cex=0.8, pch=25, pt.bg="white", add=TRUE)

plotCI(x + jit, delistedOut$RB$S50[, 'median'], li = delistedOut$RB$S50[, '25th'], ui=delistedOut$RB$S50[, '75th'], gap=0, sfrac = 0, add=TRUE, col=statusCols['g'], cex=0.8, pch=24, pt.bg="white")
...
for(j in 1:2){
	...
	# axis(side=1, at=bp, labels=FALSE, tck=-0.02)
	axis(side=1, at=bp, labels=rep(redHab, 4), tck=-0.02, cex.axis = 0.7, mgp=c(3, 0.5, -0.04))
...
axis(side=1, at=bp, labels=rep(redHab, 4), tck=-0.02, cex.axis = 0.7, mgp=c(3, 0.5, 0))
...
legend("topleft", pch = c(25, 21, 24), col=c(statusCols['r'], 1, statusCols['g']), c(expression(italic(S[25])), expression(italic(S[AVG])), expression(paste(italic(S[50])))), pt.cex = 0.8, xpd=NA, ncol = 3, bg="white")

mtext(side=1, outer=TRUE, "Percentage of subpopulations with severe declines in capacity", line=0)
