...
plotStatusDiff <- function(statusDiff){
	...
	statusDiffMat <- matrix(NA, nrow = 3, ncol = 3)
	...
	for(s in 1:2){
		axis(side = s, at = seq(1.5, 3.5, 1), c("Red", "Amber", "Green"), tck = 0, las=1)
		axis(side = s, at = c(1:4), labels=FALSE)
	}
	mtext(side = 2, line = 4, "Observed status")
	for(i in 1:3){
		for(j in 1:3){
			if(i == j) polygon(x = c(4 -i, 4-i+1, 4-i+1, 4-i), y = c(j + 1, j + 1, j, j), border = NA, col = statusCols[i])
