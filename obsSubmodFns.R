...
#' @param nYears Number of years to return monitoring coverage.
...
#' to \code{nYears} and the number of columns equal to \code{nPop}. 
...
#' 
#' samplingDesign(ppnSampled = 0.85, nPop = 35, nYears = 57, 
#' ppnChange = -0.18, samplingDeclStart = 40, samplingDeclEnd = 50, gen = 5)
#'
#' @export

samplingDesign <- function(ppnSampled, nPop, nYears, ppnChange = 0, 
													 samplingDeclStart = NULL, 
													 samplingDeclEnd = samplingDeclStart, 
													 gen = NULL){
		
	if (ppnChange == 0) { # If ppnSampled is constant
		sampled <- matrix(sample(x = c(0,1), size = nPop*nYears, replace = TRUE, 
														 prob = c(1-ppnSampled, ppnSampled)),
											nrow = nYears, ncol = nPop)
	} else {
	
		# Calculate ppnSampled for each year
		ppnSampled.change <- rep(ppnSampled, nYears)
		
		# If decline happens over time, interpolate change:
		if ((samplingDeclEnd - samplingDeclStart) > 0) {
			ppnSampled.change[(gen + 2 + samplingDeclStart) : (gen + 2 + samplingDeclEnd)] <- 
				ppnSampled + approx(x = c(0, ppnChange), n = samplingDeclEnd - samplingDeclStart + 2)$y[2:(samplingDeclEnd - samplingDeclStart + 2)]
		}
		
		# After change remain at new ppnSampled:
		ppnSampled.change[(gen + 2 + samplingDeclEnd) : nYears] <- ppnSampled + ppnChange
		...
		# and subpopulation
		sampled <- matrix(NA, nrow = nYears, ncol = nPop)
		for(y in 1:nYears){
